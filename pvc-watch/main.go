...
func main() {
	...
	flag.StringVar(&maxClaims, "max-claims", "1Gi", "Maximum total claims to watch")
	...
	// create the clientset= clienset is a serializer to connect k8s api
...
func printPVCs(pvcs *v1.PersistentVolumeClaimList) {
	...
	for _, pvc := range pvcs.Items {
		quant := pvc.Spec.Resources.Requests.Storage()
		fmt.Printf(
			template,
			pvc.Name,
